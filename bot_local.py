...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME="Sutanto The",
            BATCH_ACADEMY="Phoenix Night Online",
            GITHUB_REPO_LINK="https://github.com/sutantothe12/telebot.git"
        )
...
def send_plot(message):
    ...
    axes[0].bar(x=df_plot.index.values, height=df_plot['spent'], color='#AE2024')
    axes[0].set_ylabel('Total Spent', fontsize=8)

    #         # second subplot: total approved conversion per age group
    axes[1].bar(x=df_plot.index.values, height=df_plot['approved_conversion'], color='#000000')
    axes[1].set_ylabel('Total Approved Conversion', fontsize=8)

    # third subplot: average CPC per age group
    axes[2].bar(x=df_plot.index.values, height=df_plot['cpc'], color='#AE2024')
    ...
    # send plot
        ...
        plot_info = list(zip(
            ['total spent', 'total approved conversion', 'highest average CPC'],
            df_plot.idxmax(),
            df_plot.idxmin()))
...
def echo_all(message):
    ...
    with open('template_text/default.txt', mode='r', encoding='utf-8') as f:
        ...
        default = temp.substitute(EMOJI=emoji.emojize(":clown_face:"))
